...
import { SellerPaymentOrderService } from './seller-payment-order.service';
...
export class PaymentService {
    ...
    private async generateEntityFromInitDto(pdto: PaymentInitDTO) {
        ...
        await this.applyItemSellerSnapshots(p.items);
    ...
    public async createBillingPayment(params: {
        ...
        payment.items = [item];
        payment.taxes = [];
        // Snapshotların oluşturulması için customerAccount ve billingAddress alınır
        const customerAccount = await this.accountService.fetchOne(customerAccountId);
