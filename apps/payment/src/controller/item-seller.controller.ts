...
@Controller('/account/:accountId/item')
export class ItemSellerController {
    constructor(
    ...
    private async checkBeforAdd(account: ItemDTO) {
        if (account.entityGroup || account.entityName || account.entityId) {
            if (
                ...
                throw new BadRequestException(
                    "There is a item that have same entityGroup, entityName and entityId's. Please delete the old one to add new. you can update the old one if you wish too",
                );
            }
        }
    }
